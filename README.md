...
| `reference.py` | **The implementation** (extract the kernel from here): summary weights, dyadic builder, `pack_levels`, `multilevel_attention_forward` → `(out, lse)` with optional positional encoding (`position_mode` none/rope/relative: post-summary RoPE at interval right endpoints + learned summary-bin relative bias), explicit two-pass `multilevel_attention_backward` covering all three modes with a fixed 5-field return `(dq, dk_packed, dv_packed, dposition, stats)` -- `dposition` is the `drel_logits` seed in relative mode, else None. **Incremental decode**: `init_decode_state` (prefill hand-off), `decode_step` = `short_conv_step` (rolling pre-conv state) → merge weight → `advance_decode_state` (the one activating level merges its two live children, then the token is written at level 0) → `multilevel_attention_decode` (same positional rules) → sink → gate. The cache is per-level modular rings addressed by `range_spec` (`ring_slot`: node `j` of level `l` at `offset[l] + j % caps[l]`); no dummies, no plan replay. No oracles. |
| `minimal_reference.py` | **The semantic oracle**: deliberately naive pure-PyTorch definition of the full block — per-query visible-entry enumeration via `range_bounds` (chronological, oldest → newest), ordinary `torch.softmax`, literal sink logit + zero value row, relative bin distances literally `arange(M-1..0)`, autograd-only backward. No packed buffers, tiles, online softmax or manual backward; imports only `range_spec`. FP32 semantics only — mixed-precision op ordering stays `reference.py`'s job. Checked against `reference.py` by `test/test_reference.py`. |
| `test/test_reference.py` | **The consolidated CPU suite** (single entrypoint): Section 1 checks `range_spec` against the analytic range oracles and hand-derived goldens (range/elem/mono/fwd-exactness/inverse-contiguity/bwd-containment/domain over six schedules) plus positional-bin properties and per-geometry coverage pins; Section 2 compares two implementations selected via `--compare {mini,ref,flex}` (default `mini ref`) over a curated feature matrix (qk/linear weights, short conv, none/rope/relative, sink, gate, o_proj, softcap=None) at the frozen `(out, lse)` boundary and at the block output; Section 3 pins the packed-boundary Phase-2 backward contract (dq/dk_packed/dv_packed/dposition individually vs autograd, plus the sink-dlse seam -- the future CuTeDSL backward interface), full-block gradients (mini autograd vs the explicit composition; pure autograd for flex pairs), never-visible-node zero-grads and `detach_weights` invariants; Section 4 carries the non-comparison contracts: bf16/fp32-ordering, `ShortConv1d` module checks, negative controls (dropped dlse/tree/gate must fail; SiLU guard; sink-0 != identity), and ~60 error paths; Section 5 proves incremental decode by feeding tokens one at a time (decoded rows vs the full-sequence ref and mini rows, ring state vs a full-tree hand-off, visible-set / RoPE-position / bin-distance oracles at every step, ring capacities over whole schedules). `--compare ref flex` / `mini flex` needs torch >= 2.5 + CUDA and SKIPs cleanly otherwise. |
...
`test/test_reference.py` defines **what must still pass**. The analytic range
oracles live inside that suite and are deliberately *not* part of the clean
module.

Run everything (CPU, from `test/`):

```bash
python test_reference.py --device cpu          # mini vs ref (default pair)
python test_reference.py --compare ref flex --device cuda   # on the GPU box
```

**TODO (GPU / CuTeDSL)** -- the kernel-side suites remain to be written for
the GPU machine (needs a CUDA device and `flash_attn_4`; from `test/`).
Flex equivalence is already covered by `test_reference.py --compare ref flex`,
which anchors the kernel work to the code that actually trains:

```bash
python test_reference.py --compare ref flex --device cuda   # exists today
python test_cute_ranges.py                 # TODO: cute/ranges.py vs range_spec
python test_cute_forward.py                # TODO: fwd kernel + mask_mod vs reference
python test_cute_backward.py               # TODO: bwd kernel vs reference
...
implementation over the dyadic tree; `test/test_reference.py` checks it
against `minimal_reference.py` (the semantic oracle), the analytic range
oracles, and -- via `--compare ref flex` -- the original flex training path.
