...
Measured result (test_range_spec.py): every single node's inverse set Q_l(k)
...
def activation_times_from_fmap(fmap: Dict[int, int]) -> Tuple[int, ...]:
    ...
    Deliberately duplicates test_tiled.compute_dyadic_activation_times /
    validate_dyadic_fmap in pure ints so this module has no test dependency;
    test_range_spec asserts the two agree on every configuration.
    """
    if not fmap:
        return (0,)
    L = max(fmap)
